...
export function registerEvalRoutes(app: Express): void {
  // ─── Jalankan benchmark ────
  app.post('/v1/eval/benchmark', async (req: Request, res: Response): Promise<void> => {
    try {
  ...
  app.get('/v1/eval/benchmarks', async (_req: Request, res: Response): Promise<void> => {
    try {
      const backend: QualityBackend = await getQualityBackend();
  ...
  app.get('/v1/eval/leaderboard', async (req: Request, res: Response): Promise<void> => {
    try {
      const backend: QualityBackend = await getQualityBackend();
